...
The `0.1.1` candidate changes the source version, localizes the thunder countdown unit, makes the selected-location pin decorative, and adds a version and build timestamp to the front screen. Location coordinates and weather details remain available in adjacent text. `dist/build-info.json` records the package version, build time, timezone, source revision, and source-only clean state. Documentation now describes that record.
...
Independent correctness, security, and accessibility reviews, followed by separate votes, confirmed four fixes required before a new package build. The linked `feat/lightning-review-fixes` branch now binds build-provenance Git commands to the configuration directory, stages only the declared Electron runtime file, raises the dark-theme build-label contrast, and makes the location marker explicitly decorative instead of relying on unsupported `alt` behavior. Four focused regressions passed, for 22 passing tests overall; `npm run build` and `git diff --check` also passed on that branch. The generated `dist/` output from that local build was restored to the branch base and is not package or capture evidence. Built-surface verification and integration remain pending. Reviewers found no current packaged log or leaked local data.
...
Issue #1 tracks the remaining visual and installation checks. The next safe work is to integrate the review-fix and Android version branches into `main`, rebuild from the resulting source revision, package both platforms, and capture the new interfaces. Then retry the Sites workflow when the configured project is visible, and complete installation checks on disposable Windows and Android environments.
