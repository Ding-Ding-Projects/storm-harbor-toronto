...
The current `0.1.1` source candidate adds a localized countdown label, a descriptive name for the map location marker, visible build version and timestamp metadata, and package-version wiring for the Windows installer. Four timestamp-formatting checks and fourteen signal/data checks passed, and `npm run build` completed. That local build reports `sourceTreeClean: false`, as expected while candidate source and documentation remain uncommitted, so it is not release evidence. Android package-version wiring is ready on `feat/android-version-sync` and is not yet integrated into `main`; fresh package builds and metadata inspection remain pending. Independent correctness, security, and accessibility review confirmed four fixes are required before another package build: build-provenance Git commands must use the project directory, Windows packaging must stage only declared runtime files, the dark-theme build label needs stronger contrast, and the non-interactive location marker must be treated as decorative. Regression checks and built-surface verification are pending.

Genuine `0.1.0` captures in `evidence/` show the public map and both green and red shelter states at desktop and mobile sizes. A directly launched `0.1.0` desktop package also showed both states. The updated `0.1.1` bundles have not yet been captured, and Windows and Android install checks remain open.
