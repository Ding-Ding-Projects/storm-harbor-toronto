...
`build.bat` prepares dependencies and builds the web interface. On a prepared machine, use `npm ci`, `npm run assets`, and `npm run dev`. Use `npm run build` for the static site, `build-installer.bat` for the Windows Squirrel installer, and `npm run android:apk` for a debug APK. Android release APKs require a project-owned signing key stored outside the repository.
