...
The source revision and cleanliness commands run in the directory containing `vite.config.ts`, even when the build is launched from another working directory. `tests/build-provenance.test.mjs` imports the configuration from an unrelated temporary directory and compares both fields with the intended checkout.
