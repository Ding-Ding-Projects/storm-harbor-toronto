...
| UI-044 | Visible version and updated-at time on the front screen | `vite.config.ts` generates provenance; `src/App.tsx` renders it; `docs/distribution/build-provenance.md` explains the web and desktop metadata | English, Cantonese, and bilingual labels are provided | `tests/build-info.test.mjs` covers four timestamp cases; `tests/build-provenance.test.mjs` checks the checkout directory; `tests/interface-accessibility.test.mjs` checks dark-theme label contrast | `0.1.1` build succeeds with dirty source; clean build and fresh web, desktop, and Android captures pending | Partial |
